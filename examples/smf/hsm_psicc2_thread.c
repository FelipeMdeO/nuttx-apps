...
/********* STATE_S *********/
static void s_entry(void *o) { (void)o; printf("[psicc2] %s", __func__); }
...
static void s_exit(void *o) { (void)o; printf("%s", __func__); }

/********* STATE_S1 *********/
static void s1_entry(void *o) { (void)o; printf("%s", __func__); }
...
static enum smf_state_result s11_run(void *o)
{
  ...
  switch (obj->event.event_id) {
    case EVENT_D:
      ...
      break;
    case EVENT_G:
      printf("[psicc2] %s received EVENT_G", __func__);
      smf_set_state(SMF_CTX(obj), &demo_states[STATE_S21]);
      break;
    case EVENT_H:
      printf("[psicc2] %s received EVENT_H", __func__);
      smf_set_state(SMF_CTX(obj), &demo_states[STATE_S]);
      break;
    default:
      break;
...
static void s11_exit(void *o) { (void)o; printf("[psicc2] %s\n", __func__); }

/********* STATE_S2 *********/
static void s2_entry(void *o) { (void)o; printf("[psicc2] %s\n", __func__); }
...
int hsm_psicc2_post_event(uint32_t event_id)
{
  ...
  if (mq == (mqd_t)-1) {
    printf("psicc2][ERR] mq_open(O_WRONLY) failed: %d (did you run 'hsm_psicc2 start'?)", errno);
    return -1;
  }

  int rc = mq_send(mq, (const char *)&ev, sizeof(ev), 0);
  if (rc < 0) {
    printf("psicc2][ERR] mq_send failed: %d", errno);
    mq_close(mq);
    return -1;
  }
